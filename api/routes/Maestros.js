...
router.get('/', async (req, res, next)=> {
    ...
    try{
      ...
      const resultado = await sql.query("SELECT teacher_id, first_name, last_name, email, subject, hire_date FROM Maesstros")
...
router.get('/:id', async (req, res, next)=> {
    ...
    try{
      ...
      const resultado = await connection.request()
                          ...
                          .query("SELECT teacher_id, first_name, last_name, email, subject, hire_date FROM Maestros WHERE Maestros_id = @id")
...
router.post("/", async (req, res, next)=>{
    const Mestros = req.body;
    let resultado = {}
    try{
      let connection = await sql.connect(config)
      const result = await connection
                                .request()
                                .input("teacher_id", sql.Int, Maestros.teacher_id)
                                .input("first_name", sql.VarChar, Maestros.first_name)
                                .input("last_name", sql.VarChar, Maestros.last_name)
                                .input("email", sql.VarChar, Maestros.email)
                                .input("subject", sql.VarChar, Maestros.subject)
                                .input("hire_date", sql.VarChar, Maestros.hire_date)
    ...
    try{
      ...
      if (resultado.recordset.length > 0){
        const result = await connection
                                ...
                                .query("UPDATE Maestros SET teacher_id=@teacher_id, first_name=@first_name, last_name=@last_name,email=@email, subject=@subject, hire_date=@hire_date WHERE teacher_id=@teacher_id")
  ...
  router.delete('/:id', async (req, res, next)=> {
    ...
    try{
      ...
      if (resultado.recordset.length > 0){
        ...
        .request()
        .input("id", sql.Int, req.params.id)
        .query("DELETE from Maestros where teacher_id=@id")
         data = result.rowsAffected
      }
    }
    catch(err){
